...
import { X, Trophy, AlertCircle, CheckCircle2, Star, ArrowRight, RotateCcw } from 'lucide-react';
            ...
            >
              <div className="relative z-10 space-y-8 md:space-y-12">
                  <div className="text-center space-y-4">
                    <span className="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border bg-brand-indigo/10 border-brand-indigo text-brand-indigo">
                        ...
                        <div className="min-h-[100px] md:min-h-[120px] bg-slate-50/50 rounded-3xl p-6 md:p-8 border-4 border-dashed border-slate-200 flex flex-wrap gap-4 md:gap-8 items-center justify-center">
                            {builtSentence.map((word, idx) => (
                                <motion.button
                                    ...
                                    className="px-6 py-3 md:px-8 md:py-4 bg-brand-indigo text-white rounded-2xl font-black text-xl md:text-2xl shadow-xl hover:bg-brand-indigo/90 transition-colors tracking-wide"
                                >
                                    {word}
                                </motion.button>
                            ))}
                        ...
                        <div className="flex flex-wrap gap-4 md:gap-6 justify-center">
                            {currentExercise.options.filter(w => !builtSentence.includes(w) || currentExercise.options.filter(o => o === w).length > builtSentence.filter(o => o === w).length).map((word, idx) => (
                                <motion.button
                                    ...
                                    className="px-6 py-3 md:px-7 md:py-4 bg-white border-2 border-slate-200 rounded-2xl font-black text-xl md:text-2xl text-slate-700 shadow-sm hover:border-brand-indigo hover:text-brand-indigo transition-all tracking-wide"
                                >
                                    {word}
                                </motion.button>
                            ))}
